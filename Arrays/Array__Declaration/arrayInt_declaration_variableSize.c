...
int main()
{
    ...
    */
     ...
     *       a[b] == *(a + b) == b[a]
     *
     * @warning Intentional format mismatch:
     *          Using %d for float leads to undefined behavior.
     */
    for(loop = 0; loop < var; loop++)
    {
        printf("floatArray_G[%d] : %20d || ", loop, floatArray_G[loop]);

        // printf("[floatArray_G]%d : %20d", loop, [floatArray_G]loop); // error: expected expression before '[' token

        printf("%d[floatArray_G] : %20d\n", loop, loop[floatArray_G]);

        // printf("[%d]floatArray_G : %20d", loop, [loop]floatArray_G); // error: expected expression before '[' token
    }
     ...
     *
     * @warning Intentional format mismatch:
     *          Using %d for float results in undefined behavior.
     */
    for(loop = 0; loop < var; loop++)
    {
        printf("floatArray_L[%d] : %20d || ", loop, floatArray_L[loop]);

        // printf("[floatArray_L]%d : %20d", loop, [floatArray_L]loop); // error: expected expression before '[' token

        printf("%d[floatArray_L] : %20d\n", loop, loop[floatArray_L]);

        // printf("[%d]floatArray_L : %20d", loop, [loop]floatArray_L); // error: expected expression before '[' token
    }
 ...
 * │                                                                         │
 * │ 6. Format Specifier Issue:                                              │
 * │       %d used for float → ❌ undefined behavior                         │
 * │       Correct would be %f                                               │
 * └─────────────────────────────────────────────────────────────────────────┘
 */